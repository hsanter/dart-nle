...
first_r_loc = 34 + num_obs_types - 1 + (num_obs_types-1)*19
first_time_loc = 33 + num_obs_types - 1
...
sed_call = subprocess.run(["sed -n '" + ftl + "~19p' " + input_obs_seq +
...
sed_call = subprocess.run(
    ["sed -n '" + fol + "~19p' " + input_obs_seq], shell=True, capture_output=True)
...
# get prior mean
sed_call = subprocess.run(
    ["sed -n '" + fpml + "~19p' " + input_obs_seq], shell=True, capture_output=True)
all_pm_strings = sed_call.stdout.decode().split('\n')
all_pm = np.array(all_pm_strings[:num_obs]).astype(
    float).reshape((num_cycles, num_obs//num_cycles))
# get post mean
sed_call = subprocess.run(
    ["sed -n '" + faml + "~19p' " + input_obs_seq], shell=True, capture_output=True)
all_am_strings = sed_call.stdout.decode().split('\n')
all_am = np.array(all_am_strings[:num_obs]).astype(
    float).reshape((num_cycles, num_obs//num_cycles))
# get prior var
sed_call = subprocess.run(
    ["sed -n '" + fpvl + "~19p' " + input_obs_seq], shell=True, capture_output=True)
...
# get r estimates
sed_call = subprocess.run(
    ["sed -n '" + frl + "~19p' " + input_obs_seq], shell=True, capture_output=True)
